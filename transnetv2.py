...
def main():
    ...
    for file in args.files:
        if os.path.exists(file + ".predictions.txt") or os.path.exists(file + ".scenes.txt"):
            print(f"[TransNetV2] {file}.predictions.txt or {file}.scenes.txt already exists. Skipping video {file}.",
                  file=sys.stderr)
            continue
        ...
        throughput = len(video_frames) / latency if latency > 0 else float('inf')

        print(f"[TransNetV2] Processed {file} frames: {len(video_frames)}")
        print(f"[TransNetV2] Latency: {latency:.3f} seconds")
        print(f"[TransNetV2] Throughput: {throughput:.2f} frames/second")
        ...
        if args.visualize:
            if os.path.exists(file + ".vis.png"):
                print(f"[TransNetV2] {file}.vis.png already exists. Skipping visualization of video {file}.",
                      file=sys.stderr)
                continue
            pil_image = model.visualize_predictions(video_frames, predictions=(single_frame_predictions, all_frame_predictions))
